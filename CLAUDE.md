...
2. **Optional overview.** When the release has one unifying theme or a big headline change,
...
3. **One `#### <emoji> <title>` subsection per notable change.** Lead each with a single
   topical emoji and a short title (noun phrase or imperative). Explain the change from the
   user's point of view.

4. **`#### 🔧 Under the hood`** — collect the minor, internal, or hard-to-notice changes
   (dependency bumps, refactors, build/CI, subtle correctness fixes) here instead of giving
   each its own top-level subsection.

5. **Closing note** where warranted — e.g. a bold **Updating to this version is strongly
...
This repo is not a fork, so there is no `upstream` remote and nothing to fetch from an
original project. An unqualified `main` always means `origin`'s branch.
