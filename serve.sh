...
#
# llama-server runs behind router-shim.sh (shell + jq + socat + curl), which
# listens on $PORT and forwards to it on a private port. The shim strips
# value-constraint keywords from tool schemas so llama.cpp's tool-call grammar
# stays under its rule limit for gpt-oss and devstral; without it their Claude
# Code requests 400 with "failed to parse grammar" (see the README, and the
# shim's own header). It owns the llama-server it launches -- dies with it,
# kills it on TERM -- so router.sh still just supervises one process on $PORT.
# --host/--port for llama-server are added by the shim.
mkdir -p "$TMP/emptycache"
HF_HOME="$TMP/emptycache" exec "$DIR/router-shim.sh" \
